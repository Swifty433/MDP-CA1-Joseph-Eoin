...
World::World(sf::RenderWindow& window, FontHolder& font)
	...
	, m_spawn_position(m_camera.getSize().x / 2.f, m_world_bounds.size.y - m_camera.getSize().y / 8.f)
	, m_spawn_position_2(m_camera.getSize().x / 2.f, m_world_bounds.size.y - m_camera.getSize().y / 1.1f)
...
void World::Update(sf::Time dt)
{
	...
	{
		playerVelocity.x -= kPLayerFriction * dt.asSeconds();
...
void World::AdaptPlayerPosition()
{
	...
	//saving the oldposition to check if wall collision ever occured
	sf::Vector2f oldposition = m_player_aircraft->getPosition();
